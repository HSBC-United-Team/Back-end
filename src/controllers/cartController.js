...
const addToCart = async (req, res) => {
   ...
   try {
     ...
     const cartValidationResult = cartValidation(product_id, quantity, subtotal_price);
     ...
     if (cartValidationResult) {
       ...
       if (existingCartItem) {
         await existingCartItem.update({
           ...
           subtotal_price: existingCartItem.subtotal_price + subtotal_price,
         });
       } else {
 ...
 const updateCart = async (req, res) => {
   ...
   try {
     ...
     if (!cart) {
       return res.status(404).json({ message: "Cart tersebut tidak ada!" });
     }
 ...
 const deleteCart = async (req, res) => {
   try {
     ...
     if (isNaN(product_id) || isNaN(cart_id)) {
       return res.status(400).json({ Error: "product_id or cart_id salah" });
     }
     ...
     if (!existingCartItem) {
       return res.status(404).json({ message: "Cart item tersebut tidak ada!" });
     }
